...
[[save-event-initialization]]
== Initialization During Save Events

While UI-layer initialization events such as `InitEntityEvent` configure default values during entity creation, there are cases where initialization logic must be applied at the moment of saving. Such scenarios include applying defaults based on user input, auto-generating unique identifiers, or ensuring business rules are met before / after persisting the entity.

In Jmix, there are three common methods to execute initialization logic during the save lifecycle:

. BeforeSaveEvent (UI Controller Level): Executes just before saving within the view controller.
. EntitySavingEvent (Entity Listener Level): Fires before the entity is saved to the database, covering both UI and programmatic creations.
. EntityChangedEvent with AFTER_COMMIT (Post-Commit Level): Executes after the database commit, often used for follow-up actions or audit logging.
...
The `BeforeSaveEvent` is a view-level event that occurs before an entity is saved from the UI within a standard detail view. It is useful for setting values based on user input or computed logic from the view’s context.
...
[source,java]
----
include::example$/src/main/java/io/jmix/petclinic/view/visit/VisitDetailView.java[tags=before-save-event]
----
...
[NOTE]
====
The `BeforeSaveEvent` only triggers within the view and applies only to UI-based saves. It is not triggered by programmatic saves or REST API calls. For broader coverage, consider using `EntitySavingEvent`.
====

[[entity-saving-event]]
=== Entity Saving Event (Entity Listener Level)

The `EntitySavingEvent` is a lifecycle event triggered before an entity is saved to the database. It is a universal event that applies to all save operations, whether from the UI, generic REST API, or programmatic calls (`dataManager.save()`).

In this example, an `EntitySavingEvent` listener is used to automatically generate a unique visit number for every new `Visit` entity. This number is assigned during the save process and cannot be modified by the user.
...
Additionally, this approach prevents issues with concurrent users. If the visit number were assigned during entity initialization (e.g., using `@PostConstruct`), it could cause numbering conflicts or gaps in the sequence when multiple users create visits simultaneously.
...
<2> Sequence Generator: A unique number is generated using Jmix's built-in xref:features:sequences.adoc[Sequences API].
<3> Formatted Number:The visit number is formatted with the year and a zero-padded sequence number.
<4> Only for New Records: The number is only assigned for newly created visits (`event.isNewEntity()`), preserving existing numbers during edits.

For more information on Entity Saving Event Listener see: xref:data-access:entity-events.adoc#saving-loading-events[]

[[entity-changed-event]]
=== Entity Changed Event (After Commit)

The `EntityChangedEvent` is a lifecycle event triggered after an entity is committed to the database. It is commonly used for follow-up actions that should only occur after the database transaction is complete. Typical use cases include automatic assignments, notifications, or audit logging.

In this example, the `AssignNurseToVisitAutomaticallyEventListener` listens for `EntityChangedEvent` events and assigns an available nurse to a new visit if no nurse has been assigned:
...
<3> Event Type Check (`event.getType()`): Ensures that the logic only runs when a new visit (`Type.CREATED`) is created.
<4> `dataManager.load()` with `@Transactional`: Loads the `Visit` entity from the database in the new transaction. This is possible because of `<2>` (`Propagation.REQUIRES_NEW`). The `fetchPlan()` ensures that the assigned nurse is loaded.
<5> `assignAvailableNurseAutomatically()` Call: Proceeds with the nurse assignment logic if no nurse is assigned.

The nurse assignment happens only after the visit is fully saved in the database. Because the assignment is performed in a new transaction, it does not block or interfere with the original save operation. Additionally, this approach ensures that only nurses who are truly available (based on saved visits) are assigned, preventing double assignments or race conditions, even when multiple users create visits simultaneously.


For more information, see:
xref:data-access:entity-events.adoc#after-commit[]
