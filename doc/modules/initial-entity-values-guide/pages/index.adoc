...
[CAUTION]
====
Do not initialize fields of collection attributes with the new `ArrayList` or `HashSet` instances. It will break the xref:data-access:fetching.adoc[data fetching] functionality. If you want to have default values in the fields, use the special collections provided by Jmix: `NotInstantiatedList` or `NotInstantiatedSet`. For example:

[source,java]
----
@OneToMany(mappedBy = "user")
private List<UserStep> steps = new NotInstantiatedList<>();
----
====
...
Here’s an example of a `@PostConstruct` method in the `Visit` entity. This method sets a static default value for the treatmentStatus field when the entity is created:
